  ...
  logs
    .filter(
      (log) =>
        log.taken &&
        log.date_taken &&
        new Date(log.date_taken).getMonth() === currentMonth &&
        new Date(log.date_taken).getFullYear() === currentYear
    )
    .map((log) => format(new Date(log.date_taken!), "yyyy-MM-dd"))
);


const takenThisMonth = takenDatesSet.size;


const missedThisMonth = Array.from({ length: today }, (_, i) => {
  const dateStr = format(new Date(currentYear, currentMonth, i + 1), "yyyy-MM-dd");
  return !takenDatesSet.has(dateStr);
}).filter(Boolean).length;

const remainingThisMonth = totalDaysInMonth - today;
const monthlyAdherence = Math.round((takenThisMonth / today) * 100);
  return (
  ...
  <CardContent>
    <Progress value={monthlyAdherence} className="h-3 mb-4" />
    <div className="grid grid-cols-3 gap-4 text-center text-sm">
      <div>
        <div className="font-medium text-green-600">{takenThisMonth} days</div>
      ...
      <div>
        <div className="font-medium text-blue-600">{remainingThisMonth} days</div>
